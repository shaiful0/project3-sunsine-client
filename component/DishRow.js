import { View, Text, TouchableOpacity, Image, ImageBackgroundBase } from 'react-native'
import React, { useState } from 'react'
import tw from 'twrnc'
import { MinusCircleIcon, PlusCircleIcon } from 'react-native-heroicons/outline';
import { useDispatch, useSelector } from 'react-redux';
import { addToBasket, removeFromBasket, selectBasketItemsWithId } from '../basketSlice/basketSlice';

const DishRow = ({ dish }) => {
  const [isPressed, setIsPressed] = useState(false);
  const { _id, name, img, price, description } = dish;
      ...
      <TouchableOpacity onPress={() => setIsPressed(!isPressed)}
        ...
        <View style={tw`flex-row`}>
          <View style={tw`flex-1 pr-2`}>
            ...
            <Text style={tw`mb-1 mx-4`}>Price: {parseFloat( price)}$</Text>
          </View>
          <View>
            <Image
              source={img}
